'use client';

import { useSmoothScroll } from '../header/useSmoothScroll'

export function Footer() {
  const currentYear = new Date().getFullYear();
  const { handleClick } = useSmoothScroll();

  return (
    <footer className="bg-[#e7e6e4] pt-20 pb-8">
      <div className="container mx-auto px-6">
        {/* CTA Section */}
        <div className="flex items-center justify-between mb-20 max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-slate-900 max-w-2xl">
            Comece a monitorar seus serviços hoje
          </h2>
          <a
            href="#pricing"
            onClick={(e) => handleClick(e, '#pricing')}
            className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl font-normal text-sm hover:bg-slate-800 transition-colors border-2 border-slate-900 flex-shrink-0 cursor-pointer"
          >
            <svg
              className="w-4 h-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth={2}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M13 10V3L4 14h7v7l9-11h-7z"
              />
            </svg>
            Experimente grátis
          </a>
        </div>

        {/* Links Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-16 max-w-7xl mx-auto">
          {/* Produto */}
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 uppercase tracking-wider">
              Produto
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#features"
                  onClick={(e) => handleClick(e, '#features')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  Recursos
                </a>
              </li>
              <li>
                <a
                  href="#pricing"
                  onClick={(e) => handleClick(e, '#pricing')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  Preços
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Plataforma
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Changelog
                </a>
              </li>
            </ul>
          </div>

          {/* Soluções */}
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 uppercase tracking-wider">
              Soluções
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#usecases-by-product"
                  onClick={(e) => handleClick(e, '#usecases-by-product')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  By Product
                </a>
              </li>
              <li>
                <a
                  href="#usecases-for-founders"
                  onClick={(e) => handleClick(e, '#usecases-for-founders')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  For Founders
                </a>
              </li>
              <li>
                <a
                  href="#usecases-for-startups"
                  onClick={(e) => handleClick(e, '#usecases-for-startups')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  For Startups
                </a>
              </li>
              <li>
                <a
                  href="#usecases-for-enterprise"
                  onClick={(e) => handleClick(e, '#usecases-for-enterprise')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  For Enterprise
                </a>
              </li>
            </ul>
          </div>

          {/* Empresa */}
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 uppercase tracking-wider">
              Empresa
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Sobre nós
                </a>
              </li>
              <li>
                <a
                  href="#contact"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Contato
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Blog
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Privacidade
                </a>
              </li>
            </ul>
          </div>

          {/* Recursos */}
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 uppercase tracking-wider">
              Recursos
            </h3>
            <ul className="space-y-3">
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Documentação
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  API
                </a>
              </li>
              <li>
                <a
                  href="#faq"
                  onClick={(e) => handleClick(e, '#faq')}
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal cursor-pointer"
                >
                  FAQ
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-normal"
                >
                  Status
                </a>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-slate-300 pt-8 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            {/* Logo e Copyright */}
            <div className="flex items-center gap-8">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center">
                  <span className="text-white font-bold text-sm">L</span>
                </div>
                <span className="text-lg font-semibold text-slate-900">Lagarto</span>
              </div>
              <p className="text-sm text-slate-600 font-normal">
                © {currentYear} Lagarto. Todos os direitos reservados.
              </p>
            </div>

            {/* Social Links */}
            <div className="flex items-center gap-4">
              <a
                href="#"
                className="w-9 h-9 rounded-lg bg-slate-200 hover:bg-slate-300 flex items-center justify-center transition-colors"
                aria-label="Twitter"
              >
                <svg
                  className="w-4 h-4 text-slate-900"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-9 h-9 rounded-lg bg-slate-200 hover:bg-slate-300 flex items-center justify-center transition-colors"
                aria-label="LinkedIn"
              >
                <svg
                  className="w-4 h-4 text-slate-900"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-9 h-9 rounded-lg bg-slate-200 hover:bg-slate-300 flex items-center justify-center transition-colors"
                aria-label="GitHub"
              >
                <svg
                  className="w-4 h-4 text-slate-900"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-9 h-9 rounded-lg bg-slate-200 hover:bg-slate-300 flex items-center justify-center transition-colors"
                aria-label="Email"
              >
                <svg
                  className="w-4 h-4 text-slate-900"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  strokeWidth={2}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                  />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}